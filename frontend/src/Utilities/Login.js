...
const Login = (props) => {
    ...
    const fetchUsersAndVerifyUser = async () => {
        fetch(`http://localhost:5000/users`)
        .then(res => res.json())
        .then(response => {
            try {
                const userList = response.foundUsers;
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
        //first make sure we have valid email and username
        if (!user || !pass){
            ...
            <div className="overflow-hidden bg-white rounded-md shadow-md">
                <div className="px-4 py-6 sm:px-8 sm:py-7">
                        <div className="space-y-5">
                            ...
                            <div>
                                <button onClick={handleSubmit} className="inline-flex items-center justify-center w-full px-4 py-4 text-base font-semibold text-white transition-all duration-200 bg-blue-600 border border-transparent rounded-md focus:outline-none hover:bg-blue-700 focus:bg-blue-700">
                                    Log in
                                </button>
                            </div>

                            <div className="text-center text-blue-800 text-2xl">
